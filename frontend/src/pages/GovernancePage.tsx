          ...
          <>
            ...
            <div className="mb-4 grid gap-4 lg:grid-cols-5">
              <Panel className="lg:col-span-3">
                ...
                {overview?.alerts.length ? <div className="space-y-2">{overview.alerts.map((alert) => <div key={alert.code} className="flex gap-3 rounded-lg p-3 text-sm" style={{ background: 'var(--color-bg-secondary)', border: '1px solid var(--color-border)' }}><span className="mt-1 h-2 w-2 shrink-0 rounded-full" style={{ background: alert.level === 'high' ? 'var(--color-error)' : 'var(--color-warning)' }} /><div><div className="font-medium">{alert.code.replace(/_/g, ' ')}</div><div className="mt-1" style={{ color: 'var(--color-text-secondary)' }}>{alert.message}</div></div></div>)}</div> : <div className="flex min-h-28 items-center justify-center gap-2 text-sm" style={{ color: 'var(--color-text-secondary)' }}><CheckCircle2 size={17} style={{ color: 'var(--color-success)' }} /> No active governance alerts</div>}
              </Panel>
